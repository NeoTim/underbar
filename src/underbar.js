...
(function() {
	...
	_.last = function(array, n) {
		...
		//return n === undefined ? "hello" : "goodby";
		return n === undefined ? array.pop() : n === 0 ? [] : n > array.length ? array : array.slice(n-1);
	};
	...
	_.reject = function(collection, cb) {
		// TIP: see if you can re-use _.filter() here, without simply
		// copying code in and modifying it
		return _.filter(collection, cb, true);
	};
	...
	/*_.uniq = function(collection, isSorted, cb, context) {
		if (collection == null) return [];
	    	...
	    	for (var i = 0, length = collection.length; i < length; i++) {
	      	...
	      	if (cb) item = cb.call(context, item, i, collection);
	      	if (isSorted ? (!i || seen !== item) : !_.contains(seen, item)) {
	        		if (isSorted) seen = item;
	        		else seen.push(item);
